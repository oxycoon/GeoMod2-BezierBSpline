...
GLSceneRendererOld::GLSceneRendererOld(QQuickItem *parent)
  : QQuickItem(parent),
    _tex_size(1,1),
    _paused(true)
{
...
const QString&GLSceneRendererOld::getTexName() const {

  return _tex_name;
}

void GLSceneRendererOld::setTexName(const QString& tex_name) {

  _tex_name = tex_name;
  forceRender();
}

bool GLSceneRendererOld::isPaused() const {

  return _paused;
}

void GLSceneRendererOld::setPaused(bool paused) {

  _paused = paused;
}

void GLSceneRendererOld::forceRender() {

//  qDebug() << "Force render! : " << _tex_name;
  emit signRenderGeometryChanged(_tex_name, boundingRect() );
}

QSGNode* GLSceneRendererOld::updatePaintNode(QSGNode* old_node, QQuickItem::UpdatePaintNodeData*) {
...
void GLSceneRendererOld::itemChange(QQuickItem::ItemChange change, const QQuickItem::ItemChangeData& value) {
  ...
  if( change == QQuickItem::ItemSceneChange ) {
    ...
    if( value.window ) {
      ...
      connect( w,     &Window::signFrameReady,
               this,  &GLSceneRendererOld::update );
//      connect( this,  &GLSceneRendererOld::signRenderGeometryChanged,
//               w,     &Window::signSceneRenderGeometryChanged );
    }
  }
}

void GLSceneRendererOld::geometryChanged(const QRectF& new_geometry, const QRectF& /*oldGeometry*/) {
